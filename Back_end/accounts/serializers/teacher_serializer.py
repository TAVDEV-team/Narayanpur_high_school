...
class TeacherSerializer(serializers.ModelSerializer):
    ...
    base_subject_detail = SubjectSerializer(source='base_subject', read_only=True)
    class_teacher_of_detail = AClassSerializer(source='class_teacher_of', read_only=True)
    ...
    def validate(self, data):
        ...
        if is_class_teacher and not class_teacher_of:
            raise serializers.ValidationError(
                {"class_teacher_of": "Must be set if is_class_teacher is True."}
            )
        return data
    ...
    def create(self, validated_data):
        ...
        teacher = TeacherAccount.objects.create(account=account, **validated_data)
        return teacher
